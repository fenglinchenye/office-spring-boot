...
public class ExcelImportService extends ImportBaseService {
    ...
    private boolean verifyingDataValidity(Object object, Row row, ImportParams params,
        ...
        if (params.getVerifyHandlers() != null&&params.getVerifyHandlers().length!=0) {
            // 获取所有的校验器
            IExcelVerifyMarkHandler[] verifyHandlers = params.getVerifyHandlers();
            // 根据校验器的@Order注解值进行排序,越小越先进行校验
            List<IExcelVerifyMarkHandler> verifyHandlerList = Stream.of(verifyHandlers).sorted(((o1, o2) -> {
                Integer orderVal1 = Objects.nonNull(o1.getClass().getAnnotation(Order.class)) ? o1.getClass().getAnnotation(Order.class).value() : Integer.MAX_VALUE;
                Integer orderVal2 = Objects.nonNull(o1.getClass().getAnnotation(Order.class)) ? o1.getClass().getAnnotation(Order.class).value() : Integer.MAX_VALUE;
                return orderVal1 - orderVal2;
            })).collect(Collectors.toList());
            // 验证分组
            Class<?>[] verifyGroup;
            if (params.getVerifyGroup()!=null && params.getVerifyGroup().length!=0){
                verifyGroup = new Class<?>[]{Default.class};
            }else{
                verifyGroup = params.getVerifyGroup();
            }

            for (IExcelVerifyMarkHandler handler : verifyHandlerList) {
                ExcelVerifyMarkHandlerResult result = handler.verifyHandler(object, verifyGroup);
                if (!result.isSuccess()) {
                    if (cell == null) {
                        cell = row.createCell(row.getLastCellNum());
                    }
                    ...
                    // 设置不合法字段属性(标记字段)
                    if (object instanceof MarkExcelModel){
                        MarkExcelModel model = (MarkExcelModel) object;
                        if (CollectionUtils.isEmpty(model.getMarkProperties())){
                            model.setMarkProperties(result.getMarkPropertySet());
                        }else{
                            model.getMarkProperties().addAll(result.getMarkPropertySet());
                        }
                    }
                    isAdd = false;
                    verifyFail = true;
        ...
        if((params.isNeedVerify() || (params.getVerifyHandlers() != null&&params.getVerifyHandlers().length!=0)) && fieldErrorMsg.length() > 0){
            if (object instanceof IExcelModel) {
                IExcelModel model = (IExcelModel) object;
                model.setErrorMsg((StringUtils.isNoneBlank(model.getErrorMsg())
    ...
    public ExcelImportResult importExcelByIs(InputStream inputstream, Class<?> pojoClass,
        ...
        try {
            ...
            if (params.isNeedSave()) {
                saveThisExcelExtension(params, pojoClass, isXSSFWorkbook, book);
        }
     ...
     * @throws Exception
     */
    private void saveThisExcelExtension(ImportParams params, Class<?> pojoClass, boolean isXSSFWorkbook,
        ...
        if (ImportParams.SAVE_URL.equals(params.getSaveUrl())) {
            ...
            url = params.getSaveUrl() + File.separator + url;
        }else {
            url = params.getSaveUrl();
        }
        ...
        FileOutputStream fos = new FileOutputStream(
                url + File.separator + format.format(new Date()) + "_" + Math.round(Math.random() * 100000)
